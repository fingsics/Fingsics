...
Ball::Ball(string id, Color color, Point* positions, Matrix* rotationMatrices, int frames, bool draw, float radius) : Object(id, color, positions, rotationMatrices, frames, draw) {
    this->radius = radius;
...
Ball::Ball(string id, bool isStatic, Point pos, Point vel, Point angle, Point angularVelocity, Point force, float mass, float elasticityCoef, Color color, bool draw, float radius) :  Object(id, isStatic, pos, vel, angle, angularVelocity, force, mass, elasticityCoef, color, draw) {
    this->radius = radius;
