...
Capsule::Capsule(string id, Color color, Vector* positions, Matrix* rotationMatrices, int frames, bool draw, bool rocket, float radius, float length) : Object(id, color, positions, rotationMatrices, frames, draw) {
    this->length = length;
    this->radius = radius;
    ...
    this->obb = frames > 0 ? OBB(positions[0], Vector(radius, radius, length / 2 + radius), rotationMatrices[0]) : OBB();
}

Capsule::Capsule(string id, bool isStatic, Vector pos, Vector vel, Vector angle, Vector angularVelocity, Vector force, float mass, float elasticityCoef, Color color, bool draw, bool rocket, float radius, float length) : Object(id, isStatic, pos, vel, angle, angularVelocity, force, mass, elasticityCoef, color, draw) {
    this->length = length;
    this->radius = radius;
    this->axisDirection = rotationMatrix * Vector(0, 0, 1);
    this->drawRocket = rocket;
    ...
    this->obb = OBB(pos, Vector(radius, radius, length / 2 + radius), rotationMatrix);
}
...
void Capsule::setRotation(Matrix rotationMatrix) {
    ...
    axisDirection = rotationMatrix * Vector(0, 0, 1);
}

Vector Capsule::getAxisDirection() {
...
Vector Capsule::getCylinderPositiveEnd() {
    return position + axisDirection * length / 2;
}

Vector Capsule::getCylinderNegativeEnd() {
    return position - axisDirection * length / 2;
}
